...
def main():
    ...
    # HANDLE VOICE INPUT FROM URL PARAMETERS - UNIFIED PARAMETER NAME
    ...
    # Voice Interface Component - FIXED PARAMETER NAME
    voice_html = f"""
    ...
    document.getElementById('voiceBtn').addEventListener('click', function() {{
        ...
        voiceRecognition.onresult = function(event) {{
            ...
            if (spokenText && spokenText.length > 0) {{
                ...
                // FIXED: Use consistent parameter name 'voice_input'
    ...
    if st.session_state.conversation_history:
        ...
        for conv in reversed(st.session_state.conversation_history[-3:]):
            ...
            with st.expander(f"{icon} {conv['timestamp']} - {conv['user'][:30]}..."):
                ...
                st.write(f"**Bob:** {conv.get('bot', 'No response')}")
